...
RSpec.describe TopicsController, type: :controller do
  ...
  describe "POST #import_sections_and_topics" do
    let(:success_output) { "Import successful\nImported 5 sections" }
    let(:error_output) { "Error occurred\nImport failed" }
    let(:command_pattern) { /cd .* && bin\/rails "import:import_sections_and_topics\[.*\]" 2>&1/ }

    context "when import succeeds" do
      around do |example|
        original_process_status = $?
        status = instance_double(Process::Status, success?: true)
        Kernel.send(:remove_const, :PROCESS_STATUS) if defined?(PROCESS_STATUS)
        Kernel.const_set(:PROCESS_STATUS, status)
        $? = status
